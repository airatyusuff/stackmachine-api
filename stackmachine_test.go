...
func TestCommandShouldNotBeEmpty(t *testing.T) {
	command := ""
	_, err := StackMachine(command)

	if err == nil {
		t.Errorf("Expected error, but received %s", err)
	}
}

func TestCommandExceedsMaxInteger(t *testing.T) {
	command := "50001"
	_, err := StackMachine(command)
...
func TestCommandBelowMinInteger(t *testing.T) {
	command := "-1 -5"
	_, err := StackMachine(command)
...
func TestCommandShouldCauseOverflow(t *testing.T) {
	commands := []string{
		"50000 DUP +",
		"50000 1 +",
	}
	...
	for _, command := range commands {
		...
		fmt.Printf("result: %d", got)
	}
}

func TestCommandTooFewNumsToAdd(t *testing.T) {
	commands := []string{
		"1 +",
		"23 34 + +",
	}
